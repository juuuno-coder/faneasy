...
interface UnifiedInquiryFormProps {
  ...
  siteName?: string; // Optional override for site tracking
}
...
export default function UnifiedInquiryForm({
  ...
  // Determine current site context: explicitly passed > URL param > 'unknown'
  const currentSite = siteName || (params?.site as string) || 'unknown';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Sync to local store for immediate UI updates
      addInquiry({
        ...newInquiry,
        id: `inq-${Date.now()}`,
        createdAt: new Date(),
  ...
  const labelClass = isBold
    ? "text-sm font-bold text-(--accent) uppercase tracking-wider"
    : isClean
    ? "text-xs font-black text-gray-400 uppercase tracking-widest pl-2"
    : isTech
    ? "text-xs font-black text-(--accent) opacity-80 uppercase tracking-widest pl-2"
    : "text-sm font-medium text-gray-400";
    
  const buttonClass = isBold
    ? "w-full rounded-none bg-(--accent) py-4 font-black text-white transition-all hover:brightness-110 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed border-2 border-(--accent)"
    : isClean
    ...
    : isTech
    ? "w-full rounded-2xl bg-(--accent) py-5 font-black text-white transition-all hover:brightness-110 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_0_30px_rgba(var(--accent-rgb),0.5)]"
    : "w-full rounded-xl bg-(--accent) py-4 font-bold text-white transition-all hover:brightness-110 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_4px_20px_rgba(var(--accent-rgb),0.4)]";
